...
import static com.example.cineurubufinal.Main.listaAssentosBesouro;



public class BesouroController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        for (int i = 0; i < 100; i++) {
            if (listaAssentosBesouro.get(i).getOcupado() == Boolean.TRUE){
    ...
    public void selecionar(ActionEvent event) throws IOException {
        ...
        if (bnt.getStyle().equals("-fx-background-color: #ff1100")) {
            if (listaAssentosBesouro.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)).getOcupado()){
                System.out.println("Esse assento já está Ocupado, escolha outro!");
            }
            else {
                carrinho.remove(listaAssentosBesouro.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)));
                bnt.setStyle("-fx-background-color: #1e90ff");
            }
        }

        else {
            if (carrinho.isEmpty() && !listaAssentosBesouro.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)).getOcupado()) {

                bnt.setStyle("-fx-background-color: #ff1100");
                carrinho.add(listaAssentosBesouro.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)));
            }
            else {
    ...
    public void comprar(ActionEvent event) throws IOException, AssentoOcupadoException {
        ...
        if(tipoIngresso.getValue() != null) {
            for (Assento assento : carrinho) {
                ...
                ingresso.setFilme("Besouro Azul");
