...
import static com.example.cineurubufinal.Main.listaAssentosBarbie;


public class BarbieController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        for (int i = 0; i < 100; i++) {
            if (listaAssentosBarbie.get(i).getOcupado() == Boolean.TRUE){
    ...
    public void selecionar(ActionEvent event) throws IOException {
        ...
        if (bnt.getStyle().equals("-fx-background-color: #ff1100")) {
            if (listaAssentosBarbie.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)).getOcupado()) {
                System.out.println("Esse assento já está Ocupado, escolha outro!");
            } else {
                carrinho.remove(listaAssentosBarbie.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)));
                bnt.setStyle("-fx-background-color: #1e90ff");
            }
        }
        else {
            if (carrinho.isEmpty() && !listaAssentosBarbie.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)).getOcupado()) {
                bnt.setStyle("-fx-background-color: #ff1100");
                carrinho.add(listaAssentosBarbie.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)));
            }
            else {
