...
import static com.example.cineurubufinal.Main.listaAssentosElementos;
...
public class ElementosController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        for (int i = 0; i < 100; i++) {
            if (listaAssentosElementos.get(i).getOcupado() == Boolean.TRUE){
    ...
    public void selecionar(ActionEvent event) throws IOException {
        ...
        if (bnt.getStyle().equals("-fx-background-color: #ff1100")) {
            if (listaAssentosElementos.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)).getOcupado()){
                System.out.println("Esse assento já está Ocupado, escolha outro!");
            }
            else {
                carrinho.remove(listaAssentosElementos.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)));
                bnt.setStyle("-fx-background-color: #1e90ff");
            }
        }

        else {
            if (carrinho.isEmpty() && !listaAssentosElementos.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)).getOcupado()) {
                bnt.setStyle("-fx-background-color: #ff1100");
                carrinho.add(listaAssentosElementos.get((GridPane.getRowIndex(bnt) * 10) + GridPane.getColumnIndex(bnt)));
            }
            else {
